...
class TestXdgMua(unittest.TestCase):
    def setUp(self):
        self.xdgmua = utils.Xdg()
        self.message = email.message_from_string(message)
        self.mailto = self.xdgmua._msg_to_mailto(self.message)
        self.mdict = dict([x.split('=') for x in
            self.mailto.replace('mailto:','to=').replace('?','&').split('&')])

    def test_is_cmd(self):
        self.assertEqual(self.xdgmua.executable, "xdg-email")
        self.assertEqual(self.mailto[0:7], "mailto:")

    def test_body(self):
        self.assertTrue("body=Package%3A%20buggy-pkg" in self.mailto)
        self.assertTrue('%60ls%20%2F%60' in self.mdict['body'])
        self.assertTrue('%24%28ls%20%2Fusr%2F%29' in self.mdict['body'])
